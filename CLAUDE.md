...
- **Une vachette se lit sous trois formes.** `standing` (groupes, du moins au plus de
  points) est la seule écrite ; `ranks` (1 = le plus de points) et `points` sont lus
  indéfiniment. Pas de migration : une fusion peut en ramener une ancienne à tout moment,
  donc `vacheeGroups` doit savoir lire les trois pour toujours. Ne jamais réutiliser un de
